...
javascript: (function () {
	const w = () => {
		const b = document.getElementById('arrow-next');
		if (!b) return console.warn('Next button not found.');
		...
		o.observe(b, { attributes: true, attributeFilter: ['disabled'] });
		console.log('Waiting for button to be enabled...');
		const s = document.createElement('button');
		s.textContent = 'Stop Auto-Next';
		...
		s.style.boxShadow = '0 2px 6px rgba(0,0,0,0.2)';
		s.id = 'stop-auto-next';
		s.onclick = () => {
			o.disconnect();
		...
		function r() {
			const e = document.getElementById('stop-auto-next');
			if (e) e.remove();
		}
	};
	w();
